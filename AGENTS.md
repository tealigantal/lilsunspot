...
- Read `TASKS.md` and work only on `Current`.
- State expected files before editing.
- Implement the smallest working version and add or update tests.
...
- Update the narrowest relevant Markdown memory file for every agent task: `TASKS.md`, `lilsunspot/README-dev.md`, `lilsunspot/desktop/README.md`, or `lilsunspot/notes/*.md`. Never record secrets or private user data.
- Prefer `ignored/codex-venv` for local Python work. Before daemon-backed desktop commands, prepend `ignored/codex-venv/Scripts` and `%USERPROFILE%/.cargo/bin` to `PATH`.
- For Tauri dev, set `PYTHONPATH` to the repository root and use an ignored temporary data dir such as `ignored/tauri-dev-data`.
- Final responses include files changed, tests run, result, known risks and intentionally not done.

### Lilsunspot Done Means

- Acceptance criteria and relevant tests pass; no unnecessary Hermes core change or secret leak.
- App skeleton still starts and `scripts/check.ps1` passes, or an unrelated failure is explicitly documented.
- Installer-impacting work produces a fresh NSIS `setup.exe` with its path reported, or records the exact blocker.
- macOS packaging work defines both native-architecture DMG jobs and installed-app smoke checks; cloud-only results are not passed until GitHub Actions produces the artifacts.
