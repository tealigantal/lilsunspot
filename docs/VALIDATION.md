...
- CI recovery: first PR run exposed a three-part workflow contract drift: macOS test jobs omitted the official `messaging` extra required by the new Weixin capability check, release tests passed an undeclared `pytest-timeout` option, and the macOS-only guard forbade the Windows release checker that this task must update. The workflow now installs `messaging` consistently, the release checker has no undeclared plugin parameter, and the guard protects only the independent release-chain files. The exact locked CI-equivalent command passes locally: `uv run --locked --extra web --extra lilsunspot --extra messaging --extra dev python -m pytest lilsunspot/daemon/tests lilsunspot/tests --basetemp ...` -> `218 passed`; secret guard and `git diff --check` also passed. Cloud rerun is the remaining final gate.
- CI boundary recovery: the fixed upstream import preserves 8,485 official commits and their authors. Existing global CI treated that preserved history as locally authored code, reporting 2,701 pre-existing upstream Windows-footgun findings and attempting to map every upstream author. The exception is constrained to a PR that actually advances an ancestor-valid `lilsunspot/UPSTREAM_COMMIT.txt`: footgun checking scans only the delta after that fixed object and attribution excludes that object history. Ordinary PRs still use the original full-repository and complete-range checks. The local release integrator email is mapped independently.
- Delta verification found two `os.kill` checks in the intentionally macOS-only DMG smoke runner. They are now explicitly annotated as platform-gated; the post-target footgun command then has no remaining findings.
- First cloud execution of that boundary failed closed because the reusable lint workflow used a shallow checkout, so neither the PR base ref nor the fixed object could be proven reachable. The checkout now requests full history; the same local ancestry and delta scan has already passed.
- Final PR acceptance: [PR #36](https://github.com/tealigantal/lilsunspot/pull/36) at `23c86c6670` passed the installed Windows regression (`15m48s`, including NSIS rebuild, Rust tests and temporary setup.exe smoke) and both native private macOS DMG jobs (`arm64 7m9s`, `x86_64 16m41s`, each including installed-app functional smoke and artifact upload). CodeRabbit was intentionally skipped because the PR remains draft.
