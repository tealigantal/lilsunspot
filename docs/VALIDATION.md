...
- CI recovery: first PR run exposed an outdated macOS workflow contract: it passed a `pytest-timeout` option without that dependency and forbade the two Windows release scripts intentionally changed by this task. Removed the unsupported option and narrowed the macOS-only protection list; then synchronized macOS sidecar collection/assets with Windows. The exact CI command now passes locally: `uv run --locked --extra web --extra lilsunspot --extra dev python -m pytest lilsunspot/daemon/tests lilsunspot/tests --basetemp ...` -> `218 passed`.
